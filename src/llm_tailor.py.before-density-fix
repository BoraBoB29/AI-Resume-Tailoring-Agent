...
_env_int = env_int
_is_retryable_mistral_error = is_retryable_error
_complete_with_retries = complete_with_retries


# ============================================================
# PER-COMPANY EXPERIENCE BULLET TARGETS
#
# This tool currently tailors one specific candidate's resume, so these
# targets are matched by company name rather than derived from the master
# resume/JD. To add or change a company's bullet target, edit this dict --
# it is the single source of truth (previously duplicated in two separate
# floor/cap blocks below). Matching is a case-insensitive substring check
# against the company name in the master resume.
# ============================================================

_EXPERIENCE_BULLET_TARGETS = {
    "tracelink": 7,
    "emotorad": 6,
}
_DEFAULT_EXPERIENCE_BULLET_TARGET = 4


def _experience_bullet_target(company: str) -> int:
    company = (company or "").casefold()
    for name, target in _EXPERIENCE_BULLET_TARGETS.items():
        if name in company:
            return target
...
class ResumeTailor:
    ...
    def _fallback_skills(self, master_resume):
        ...
        if "categories" in source_skills:
            ...
            if isinstance(categories, dict):
                return self._compact_skill_categories(categories)

        return self._compact_skill_categories(source_skills)

    def _compact_skill_categories(
        self,
        categories,
        max_categories=6,
        max_values=8
        ...
        The category names are intentionally generalized (not employer/JD-specific).
        Values are sourced only from the supplied resume data; no skills are invented.
        """
        ...
        aliases = {
            ...
            "enterprise platforms": "Systems & Platforms",
            "data and tools": "Data & Analytics",
            "data & tools": "Data & Analytics",
            "data and programming": "Data & Analytics",
            "reporting and visualization": "Reporting & Visualization",
            ...
            "collaboration and tools": "Collaboration & Tools",
            "stakeholder collaboration": "Stakeholder Collaboration",
            "client & stakeholder management": "Stakeholder Collaboration",
            "client and stakeholder management": "Stakeholder Collaboration",
        ...
        merged = {}
        source_order = []
        for raw_category, values in categories.items():
            if not isinstance(values, list):
                continue
            category = str(raw_category).strip()
            normalized = aliases.get(category.casefold(), category)
            if normalized not in merged:
                merged[normalized] = []
                source_order.append(normalized)
            for value in values:
                text = str(value).strip()
                if not text or text.casefold() == "workato":
                    continue
                if text.casefold() not in {v.casefold() for v in merged[normalized]}:
                    merged[normalized].append(text)

        # Keep the LLM's useful category names, but collapse excess categories
        # into the most relevant existing/generalized bucket instead of creating
        # an "Additional Skills" section.
        ordered = [c for c in preferred_order if c in merged] + [
            c for c in source_order if c not in preferred_order
        ]
        result = {}
        for category in ordered:
            values = merged.get(category, [])
            if not values:
                continue
            if len(result) < max_categories:
                result[category] = values[:max_values]
            else:
                target = next(iter(result), None)
                if target:
                    for value in values:
                        if value.casefold() not in {v.casefold() for v in result[target]}:
                            result[target].append(value)
                            if len(result[target]) >= max_values:
                                break
    ...
    def _normalize_skills(
        ...
        if not skills:
            ...
            canonical = self._fallback_skills(master_resume)
            data["skills"] = {
                "categories": self._compact_skill_categories(canonical)
            }
        ...
        if isinstance(
            ...
            canonical_categories = self._fallback_skills(
                master_resume
            )

            for category, values in canonical_categories.items():
            ...
            data["skills"] = {
                "categories": self._compact_skill_categories(categories)
            }
        ...
        data["skills"] = {
            "categories":
                self._compact_skill_categories(
                    self._fallback_skills(master_resume)
                )
        }
    ...
    def _ensure_experience_content_floor(
        ...
        for exp in result_experience:
            ...
            target = _experience_bullet_target(company)
            ...
            # Uses the same target as the floor above, so the
            # cap and the backfill can never disagree.
            # ------------------------------------------------

            current_bullets = current_bullets[:target]
    ...
    def _normalize_projects(
        ...
        for project in projects:
            ...
            for bullet in canonical_bullets:
                ...
                if bullet and bullet not in current["bullets"]:
                    current["bullets"].append(bullet)
                if len(current["bullets"]) >= 2:
                    break
    ...
    #
    # NOTE: this enforces 2-3 bullets per project for validation/evidence-
    # matching purposes (see evidence_matcher.py, unsupported_claims.py) and
    # to keep room for a future multi-bullet layout. The current one-page
    # template (latex_renderer.format_projects) intentionally renders only
    # the FIRST bullet as the visible project description -- bullets 2-3
    # are not wasted effort, they back the ATS/evidence checks even though
    # they aren't printed on the PDF today.
    # ========================================================
...
8. Each project: exactly 1 concise description (1–2 lines maximum).

9. Skills: 5–6 generalized categories; never use an employer name or an "Additional Skills" category.
